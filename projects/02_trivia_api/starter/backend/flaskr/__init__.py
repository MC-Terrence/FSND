...
#get all questions or search result if any
  ...
  def retrieve_questions():
    search = request.args.get('search', None, type=str) #get search criteria from request argument if available
    if search: #if search criteria is present
      ...
      current_questions = paginate_questions(request, selection) #matching search term
    ...
    else: #else display all questions paginated
  ...
  # delete question based on question id referenced.
  @app.route('/questions/<int:question_id>', methods = ['DELETE']) #Delete question based on Question ID entered
  ...
  def retrieve_questions_per_category(category_id):
    try:
      selection = db.session.query(Question).filter(Question.category==category_id).all()
      current_questions = paginate_questions(request, selection)
      return jsonify({
        'success': True,
        'total_questions': len(selection),
    ...
    try:
      ...
      if previous_questions is None: #no previous question output
        if trivia_category: #if category selected
          category_questions = db.session.query(Question).filter(Question.category==trivia_category).all()
        ...
        if trivia_category: # if category selected
          category_questions = db.session.query(Question).filter(Question.category==trivia_category).filter(Question.id.notin_(previous_questions)).all()
          random_question = random.choice(category_questions)
          return jsonify({
            'success': True,
            'question': random_question.question
          ...
          category_questions = db.session.query(Question).filter(Question.id.notin_(previous_questions)).all()
          random_question = random.choice(category_questions)
          return jsonify({
            'success': True,
            'question': random_question.question
